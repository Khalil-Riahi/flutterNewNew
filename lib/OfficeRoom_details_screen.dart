...
class _OfficeRoomReservationScreenState extends State<OfficeRoomReservationScreen> {
  ...
  DateTime get nowTunisia => DateTime.now().toUtc().add(const Duration(hours: 1));
  DateTime focusedDay = DateTime.now().toUtc().add(const Duration(hours: 1));

  @override
  void initState() {
    super.initState();
    selectedDate = DateTime(nowTunisia.year, nowTunisia.month, nowTunisia.day);
    _generateTimeSlots();
    _fetchUserPoints();
  ...
  void _generateTimeSlots() {
    _allTimeSlots.clear();
    final now = nowTunisia;
    final today = DateTime(now.year, now.month, now.day);
    final isToday = selectedDate != null &&
        DateTime(selectedDate!.year, selectedDate!.month, selectedDate!.day) == today;
  ...
  Future<void> _fetchReservationsForDate(DateTime date) async {
    ...
    try {
      final resp = await http.get(
        Uri.parse('http://localhost:8000/ELACO/booking/getReservationPrivateOffice'),
        headers: {'Content-Type': 'application/json'},
      );
      ...
      if (resp.statusCode == 200) {
        ...
        setState(() {
          reservations = allReservations.where((res) {
            try {
              final resDateStr = res['date'].toString().split('T')[0];
              final resDate = DateTime.parse(resDateStr).toUtc().add(const Duration(hours: 1));
              ...
              children: [
                Card(
                  ...
                  child: Padding(
                    ...
                    child: TableCalendar(
                      firstDay: nowTunisia,
                      lastDay: nowTunisia.add(const Duration(days: 365)),
                      focusedDay: focusedDay,
                      selectedDayPredicate: (day) => day == selectedDate,
